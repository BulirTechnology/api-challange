...
@ApiTags("Wallet")
@Controller("/credits")
export class FetchCreditTransactionHistoryController {
  constructor(
    private fetchUserTransactionCreditHistoryUseCase: FetchUserTransactionCreditHistoryUseCase
  ) { }
  ...
  async handle(
    ...
    const result = await this.fetchUserTransactionCreditHistoryUseCase.execute({
      userId: user.sub,
