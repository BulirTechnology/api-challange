...
type UpdatePhoneNumberBodySchema = z.infer<typeof updatePhoneNumberBodySchema>

@ApiTags("Users")
@Controller("/users")
export class UpdatePhoneNumberController {
  constructor(
    private updatePhoneNumber: UpdatePhoneNumberUseCase,
  ...
  async handle(
    ...
    @Body() data: UpdatePhoneNumberBodySchema
  ) {
    try {
      await this.validation.validateData(updatePhoneNumberBodySchema, data);
    } catch (error) {
